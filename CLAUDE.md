...
Build Windows preview: `.\run_windows.ps1` (requires Visual Studio C++ tools)
...
For UI review on laptop without Android hardware:

```powershell
.\run_windows.ps1
```

This swaps in stub packages (`packages/usb_serial_stub`, `packages/flutter_bluetooth_serial_stub`), launches the full app on Windows, then restores the Android pubspec. All USB/Bluetooth calls return empty/null — no ECU connection, UI only.

**Requires:** Visual Studio 2022 with "Desktop development with C++" workload.

Key files:
- `pubspec_windows.yaml` — pubspec with `dependency_overrides` for stubs
- `packages/usb_serial_stub/` — pure Dart stub matching `usb_serial` API
- `packages/flutter_bluetooth_serial_stub/` — pure Dart stub matching `flutter_bluetooth_serial` API
- `windows/` — Flutter Windows platform files (generated by `flutter create --platforms=windows`)

---

## Known Build Fixes (Applied to Pub Cache — 2026-05-27)

These two packages have old `build.gradle` files incompatible with AGP 9 + Kotlin 2.x. They were patched directly in the pub cache:

### `flutter_bluetooth_serial-0.4.0`
File: `~/.pub-cache/hosted/pub.dev/flutter_bluetooth_serial-0.4.0/android/build.gradle`
- Removed `buildscript {}` block (AGP 9 ignores subproject buildscript blocks)
- Removed `rootProject.allprojects {}` block (deprecated in AGP 9)
- Updated `compileSdkVersion 30` → `compileSdk 34`
- Added `namespace 'io.github.edufolly.flutterbluetoothserial'`
- Removed `buildToolsVersion '30.0.3'`

### `usb_serial-0.4.0`
File: `~/.pub-cache/hosted/pub.dev/usb_serial-0.4.0/android/build.gradle`
- Same removals as above
- Moved JitPack repository to project-level `repositories {}` block
- Updated `compileSdkVersion 29` → `compileSdk 34`, `minSdkVersion 16` → `minSdk 19`
- Added `namespace 'dev.bessems.usbserial'`

JitPack (`https://jitpack.io`) also added to `android/build.gradle.kts` allprojects repositories — required for `usb_serial`'s `com.github.felHR85:UsbSerial:6.1.0` dependency.

### `web_landing_screen.dart` — `dart:html` Conditional Import
- `dart:html` is web-only and caused APK compile failure
- Fixed with conditional import via `lib/screens/web/web_open_url.dart` → `web_open_url_web.dart` (web) / `web_open_url_io.dart` (all other platforms)
